...
import { togglePostCommentLike, submitPostCommentReply, getPostCommentReplies } from "../api/postService";

export const usePostCommentList = (initialComments: PostComment[]) => {
    ...
    const [replyContent, setReplyContent] = useState<Record<number, PostComment[]>>({});

    const commentLikeToggleHook = async (commentId: number): Promise<void> => {
        const res = await togglePostCommentLike(commentId);
    ...
    const commentReplySubmitHook = async (
        ...
        const res = await submitPostCommentReply(req.commentReplyId, req.commentReplyText);
        if (!res.ok) {
    ...
    const onReplyCommentReqHook = async (commentId: number) => {
        const res = await getPostCommentReplies(commentId);
